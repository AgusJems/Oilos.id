...
export default function ListBarangTable() {
  const [ListBarangData, setListBarangData] = useState<ListBarangItem[]>([]);
  ...
  const closeModal = () => {
    setIsOpen(false);
    setFormData({ name: "", description: "", image: "", price: "", status: true });
    setEditingId(null);
  };
  ...
  const fetchListBarang = () => {
    fetch("http://localhost:3001/api/getAllItems")
      .then((res) => res.json())
      .then((data) => setListBarangData(data.data))
  ...
  const handleAddListBarang = async () => {
    try {
      ...
      const res = await fetch("http://localhost:3001/api/insertDetailItems", {
        method: "POST",
      ...
      if (res.ok) {
        showSuccess("Berhasil", "Barang berhasil ditambahkan.");
        setFormData({ name: "", description: "", image: "", price:"", status: true });
        setIsOpen(false);
        fetchListBarang();
      } else {
        showError("Gagal", "Gagal menambahkan barang.");
      }
    } catch (error) {
      closeSwal();
      showError("Error", "Terjadi kesalahan saat menambahkan barang.");
    }
  };

  const handleEditListBarang = (ListBarang: ListBarangItem) => {
    setFormData({
      name: ListBarang.name,
      description: ListBarang.description,
      image: ListBarang.image,
      price: ListBarang.price.toString(),
      status: ListBarang.status === 1
    });
    setEditingId(ListBarang.id);
    setIsOpen(true);
  };

  const handleUpdateListBarang = async () => {
    try {
      showLoading("Mengupdate barang...");
      const res = await fetch(`http://localhost:3001/api/updateItems/${editingId}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(formData),
      });
      ...
      if (res.ok) {
        showSuccess("Berhasil", "Barang berhasil diperbarui.");
        setFormData({ name: "", description: "", image: "", price:"", status: true });
        setIsOpen(false);
        setEditingId(null);
        fetchListBarang();
      } else {
        showError("Gagal", "Gagal mengupdate barang.");
      }
    } catch (error) {
      closeSwal();
      showError("Error", "Terjadi kesalahan saat update barang.");
    }
  };

  const handleSubmitListBarang = () => {
    if (editingId) {
      handleUpdateListBarang();
    } else {
      handleAddListBarang();
    }
  };

  const handleDeleteListBarang = async (id: number) => {
  const confirmDelete = await showConfirmDelete(
    ...
    try {
      ...
      const res = await fetch(`http://localhost:3001/api/deleteItems/${id}`, {
        method: "DELETE",
      });
      closeSwal();

      if (res.ok) {
        showSuccess("Berhasil", "Barang berhasil dihapus.");
        fetchListBarang();
      } else {
        showError("Gagal", "Gagal menghapus barang.");
      }
    } catch (error) {
      closeSwal();
      showError("Error", "Terjadi kesalahan saat menghapus barang.");
    }
  };


  const totalPages = Math.ceil(ListBarangData.length / itemsPerPage);
  const paginatedListBarang = ListBarangData.slice(
      ...
      <div className="max-w-full overflow-x-auto">
        ...
        <Table>
          <TableHeader className="border-b border-gray-100 dark:border-white/[0.05]">
            <TableRow>
              <TableCell isHeader className="px-5 py-3 font-medium text-gray-500 text-center text-theme-xs dark:text-gray-400">Nama Barang</TableCell>
              ...
              <TableCell isHeader className="px-5 py-3 font-medium text-gray-500 text-center text-theme-xs dark:text-gray-400">Harga</TableCell>
          ...
          <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
            {paginatedListBarang.map((item) => (
              <TableRow key={item.id}>
                ...
                <TableCell className="px-4 py-4 sm:px-6 text-start">
                  <span className="block font-medium text-gray-800 text-theme-sm dark:text-white/90">{item.price}</span>
                </TableCell>
                <TableCell className="px-4 py-4 sm:px-6">
                  <div className="flex justify-center gap-2">
                    <button onClick={() => handleEditListBarang(item)} className="flex w-full items-center justify-center gap-2 rounded-full border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 shadow-theme-xs hover:bg-gradient-to-r from-blue-500 to-blue-400 hover:text-white dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03] dark:hover:text-gray-200 lg:inline-flex lg:w-auto">
                    <svg
                  ...
                  <button onClick={() => handleDeleteListBarang(item.id)} className="flex w-full items-center justify-center gap-2 rounded-full border border-red-300 bg-white px-4 py-2 text-sm font-medium text-red-700 shadow-theme-xs hover:bg-gradient-to-r from-red-500 to-red-400 hover:text-white dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03] dark:hover:text-gray-200 lg:inline-flex lg:w-auto">
                    <svg
                        className="fill-current"
                        width="18"
                        height="18"
    ...
    <Modal isOpen={isOpen} onClose={closeModal} className="max-w-[1000px] m-4">
      <div className="relative w-full p-4 overflow-y-auto bg-white rounded-3xl dark:bg-gray-900 lg:p-11">
        ...
        <div className="flex items-center gap-3 px-2 mt-6 lg:justify-end">
          ...
          <Button size="sm" onClick={handleSubmitListBarang}>
